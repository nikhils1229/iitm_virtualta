...
import requests # Import requests for web API calls
    ...
    try:
        ...
        if search_results and search_results[0]['distance'] is not None:
            if search_results[0]['distance'] < RELEVANCE_THRESHOLD:
                ...
                print(f"DEBUG: Relevant internal results found (best distance: {search_results[0]['distance']:.2f} < {RELEVANCE_THRESHOLD}). Skipping web search.")
            else:
                print(f"DEBUG: Internal results found but not relevant enough (best distance: {search_results[0]['distance']:.2f} >= {RELEVANCE_THRESHOLD}). Attempting web search...")
        else:
            print(f"DEBUG: No internal results found for '{question}'. Attempting web search...")
        ...
        if not internal_knowledge_found:
            ...
            if not web_search_results_text:
                print(f"DEBUG: No significant web search results found for '{question}'. (web_search_results_text is empty or None)")
            else:
                print(f"DEBUG: Web search successful. Length of web_search_results_text: {len(web_search_results_text)}")
        else:
            print("DEBUG: Web search not performed (internal knowledge was relevant).")
        ...
        links = []
        print(f"DEBUG: Links initialized: {links}")

        # Priority 1: Add links from relevant internal knowledge
        if internal_knowledge_found:
            for result in search_results:
                if result['url'] and result['text'] and result['distance'] is not None and result['distance'] < RELEVANCE_THRESHOLD:
                    links.append({
                        "url": result['url'],
                        "text": result['text']
                    })
            if not links: # Fallback if relevant internal results had no valid URLs/text
                 links.append({
                    "url": "https://tds.s-anand.net/",
                    "text": "TDS Course Materials (Based on internal search, but no direct link)"
                })
            print(f"DEBUG: Links after internal knowledge block: {links}")
        
        # Priority 2: If no relevant internal links, but web search was performed, add a general web search link
        # This condition ensures we don't add a web search link if internal links are already present.
        print(f"DEBUG: Checking web search link condition: web_search_results_text={bool(web_search_results_text)}, not links={not links}")
        ...
        print(f"DEBUG: Links after web search link block: {links}")
        ...
        print(f"DEBUG: Links at final stage: {links}")
