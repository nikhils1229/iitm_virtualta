...
def answer_question():
    try:
        ...
        # --- MODIFIED LOGIC: Prioritize internal knowledge based on RELEVANCE_THRESHOLD ---
        search_results = search_knowledge_base(question, n_results=3)

        # A lower distance means higher relevance. Max distance can be 2 for cosine, 0 for exact match.
        # Adjust this value as needed. For example, 0.5 to 0.7 are common ranges for 'relevant enough'.
        RELEVANCE_THRESHOLD = 0.6

        internal_knowledge_found = False
        if search_results and search_results[0]['distance'] is not None:
            # Check if the closest document is within the relevance threshold
        ...
        # Format links - Now more intelligently based on where the answer came from
        links = []
        if internal_knowledge_found:
            # Only add internal links that were considered relevant
        ...
        # Add a general web search link if external search was performed AND no relevant internal links were added
        # This prevents showing both external and irrelevant internal links for the same answer
        ...
        elif not links: # If absolutely no information was found (neither internal nor external)
            links.append({
                "url": "https://tds.s-anand.net/",
